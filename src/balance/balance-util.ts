...
export const getPrivateDisplayBalances = async (chainName: NetworkName) => {
  ...
  const header = `${CHAIN_NAME.green} ${balanceType} BALANCES`;
  ...
  if (balances.length === 0) {
    ...
    display.push(`${balanceHeader} ${balanceType} Balances...`.grey);
